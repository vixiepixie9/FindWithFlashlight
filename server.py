...
def get_items():
    ...
    df = pd.read_excel(file_path, skiprows=1, usecols=[1,2,3])  # Чете Excel файла
    df.columns = ["name", "correct", "image"]
    items = df.to_dict(orient="records")  # Преобразува в JSON формат
    return jsonify(items)  # Връща данните като JSON
